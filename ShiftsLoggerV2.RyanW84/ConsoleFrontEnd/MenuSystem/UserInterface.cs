...
public class UserInterface
{
    ...
    // Fix for CS1061: 'string' does not contain a definition for 'SortOrder'
    // The issue is that `filterCriteria` is a string, but the code is trying to access a property `SortOrder` on it.
    // The correct object to set `SortOrder` is `filterOptions`, not `filterCriteria`.
