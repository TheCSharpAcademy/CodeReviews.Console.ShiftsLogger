...
using Shift = ShiftsLogger.Ryanw84.Models.Shift;
...
public class ShiftService : IShiftService
{
    ...
    public async Task<ApiResponseDto<List<Shift>>> GetAllShifts(ShiftOptions shiftOptions)
    {
        var query = _dbContext
            ...
            .Include(s => s.Location) // Include the LocationId navigation property
            .AsQueryable();
        ...
        if (!string.IsNullOrWhiteSpace(shiftOptions.WorkerName))
        {
            query = query.Where(s =>
                s.Worker != null
                && s.Worker.Name != null
                && s.Worker.Name.Equals(shiftOptions.WorkerName, StringComparison.OrdinalIgnoreCase)
            ); // Corrected filter to ignore case
        }
        ...
        if (!string.IsNullOrEmpty(shiftOptions.LocationName))
        {
            query = query.Where(s =>
                ...
                && s.Location.Name == shiftOptions.LocationName
                    ...
                    query =
                        shiftOptions.SortOrder == "ASC"
                            ...
                            : query.OrderByDescending(s =>
                                s.Worker != null ? s.Worker.Name : string.Empty
                            );
                    break;
                case "location_name":
                    query =
                        shiftOptions.SortOrder == "ASC"
                            ? query.OrderBy(s =>
                                s.Location != null ? s.Location.Name : string.Empty
                            )
                            : query.OrderByDescending(s =>
                                s.Location != null ? s.Location.Name : string.Empty
                            );
        ...
        if (!string.IsNullOrEmpty(shiftOptions.Search))
        {
            ...
            shifts = data.Where(s =>
                    searchChars.All(c =>
                        (s.Worker.Name != null && s.Worker.Name.ToLower().Contains(c))
                        || (
                            s.Worker != null
                            && s.Worker.Name != null
                            && s.Worker.Name.ToLower().Contains(c)
                        )
                        || (
                            s.Location != null
                            && s.Location.Name != null
                            && s.Location.Name.ToLower().Contains(c)
                        )
            ...
            shifts =
                data.Skip(count: (shiftOptions.PageNumber - 1) * shiftOptions.PageSize)
                    .Take(shiftOptions.PageSize)
                    .ToList() as List<Shift>;
        }
        else
        {
            query = query
    ...
    {
        var result = await _dbContext.Shift.FirstOrDefaultAsync(s => s.Id == id);
